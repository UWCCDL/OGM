...
# Memory Network and Spreading Activation

## Generating Memory Network

The first step is to create our memories. 

![Autobiographical Memory](ESK.png)
...
I am interested in how long the agent chooses to retrieve before terminating, so I initialized an environment and agent, and simulated the agent attempting to retrieve 20 times and displayed the number of nodes encountered. Memory decay is disabled for now.
...
First, visualizing the retrieval lengths of an agent over ten time units, using a new environment and agent for ten repetitions.
...
Next, plotting average and final retrieval lengths across different parameters. The defaults I used were $\alpha = 0.1$, $\gamma = 0.9$, and $temp = 0.1$. 
